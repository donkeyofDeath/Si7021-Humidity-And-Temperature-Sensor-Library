...
float SI7021::rhCalc(uint16_t sensorValue){
		return (((125 * (float) (sensorValue)) / 65536) - 6);
}

float SI7021::tempCalc(uint16_t sensorValue){
...
{	
	if(reg==0xE0){
	    ...
	    if(computeCRC8(receivedBytes)==checkSum){
	    	...
	    	communicationSuccessFlag=true;
	    } 
	    else{
	    	communicationSuccessFlag=false;
	    }
	    //Serial.println(var);
	    return var;
	}
